...
func (c *Connection) init(impl *networkImpl, addr *net.UDPAddr) {
	c.networkImpl = impl
	c.Conn = impl.socket
	c.Addr = addr
...
func (c *Connection) keepAlive() {
	...
	for {
		...
		if currentTime-c.lastReceivedTime > int64(CfgTimeoutThreshold) || c.GetPing() > CfgMaxPing {
			...
			go func() {
				defer antiPanic(nil)
				c.networkImpl.disconnectClient(c, disconnectTypeTimeout, nil)
			}()
		}
	}
}
...
func (c *Connection) process(packet *Packet, channel Channel) {
	if packet.data != nil && len(packet.data) > 0 {
		invokePacketCallback(c.networkImpl.onPacket, c, packet.data, channel)
	}
}
...
func (c *Connection) processSend(packet *Packet, resend bool) {
	...
	c.networkImpl.writeFunc(c.Conn, c.Addr, buffer)
